...
public class EmbeddedLibraryTools {
    ...
    public static String[] defineLibs() {
        String osName = System.getProperty("os.name");
        if (osName.toLowerCase().contains("windows")) {
            return new String[]{"libwinpthread-1.dll", "libgcc_s_seh-1.dll", "libstdc++-6.dll", "libquadmath-0.dll", "libgfortran-5.dll", "liblid.dll"};
        } else if (osName.toLowerCase().contains("mac os x")) {
            return new String[]{};
        } else {
            return new String[]{/*"libc.so.6","libpthread.so.0", "libgcc_s.so.1", "libstdc++.so.6", "libz.so.1", "libm.so.6", */"libquadmath.so.0", "libgfortran.so.5", "liblid.so"};
        }
    }
...
//                System.out.println(name);
                nativeLibraryUrl = lidJNI.class.getResource(name);
