...
typedef struct VoskLidModel LidModel;
...
%extend LidModel {
    LidModel(const char *model_path)  {
        return vosk_lid_model_new(model_path);
    }
    ~LidModel() {
        vosk_lid_model_free($self);
    }
}
...
%extend KaldiRecognizer {
    ...
    KaldiRecognizer(Model *model, LidModel *lid_model, float sample_rate)  {
        return vosk_recognizer_new_lid(model, lid_model, sample_rate);
    }
